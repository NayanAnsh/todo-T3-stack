            ...
            <div className="w-1/3 border-r-2 h-screen ">
              ...
              <div className="relative   ">
                ...
                <Button variant={"sidebar"} >Tasks</Button>
                  ...
                  return<Link key={index} className="w-full px-2" href={`../folder/${folder.name.trim()}`} > <Button className="w-full" key={index} variant={"sidebar"} >{folder.name}</Button> </Link>
                  }) }
                  
                  
                  <Button variant={"sidebar"} className="transition-all" onClick={handleAddFolderClick}  >
                    <BsPlusCircleDotted size={30} />
                  </Button>
        <div className={`${!isFolderAddFormOpen ? "hidden transition-all m-4" : ""}`}>

          <Form {...Folderform} >
          <form onSubmit={Folderform.handleSubmit(onSubmit)} className="space-y-6">
